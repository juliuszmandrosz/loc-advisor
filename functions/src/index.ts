...
export const getLocalRecommendations = functions.https.onRequest(async (req, res) => {
    try {
        ...
        if (!location) {
            res.status(400).send({error: "Location is required in the request body."});
            return;
        }
        ...
        const generatePrompt = (location: string, preferences?: string) => {
            ...
            ${preferences ? `Preferencje użytkownika to: ${preferences}.` : "brak"}
